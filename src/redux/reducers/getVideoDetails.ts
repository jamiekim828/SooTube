...
export const getVideoDetails = createAsyncThunk(
  ...
  async (id: string) => {
    const {
      ...
      `${SOOTUBE_API_URL}/videos?key=${API_KEY}&part=snippet,statistics&type=video&id=${id}`
    );
...
const parseData = async (item: {
  ...
  const {
    ...
    `${SOOTUBE_API_URL}/channels?part=snippet,statistics&id=${item.snippet.channelId}&key=${API_KEY}`
  );
